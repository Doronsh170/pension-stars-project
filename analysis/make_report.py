...
w("- **קטגוריה** = סיווג הקופה + מסלול ההשקעה (למשל *קרנות השתלמות – כללי*), "
  ...
  "בשנת האיתות.")
w("- **זיקוק המסלולים:** ממסלול *כללי* הוצאו קרנות סל/מחקות-מדד של אינדקס בודד "
  "(עוקבות S&P 500, ת\"א 25, מדדי אג\"ח וכד') שסווגו \"כללי\" אך אינן קופה מעורבת "
  "ואף הובילו אותו ב-2014–2015; ומסלול *מניות* פוצל לפי גיאוגרפיה (ישראל / חו\"ל / "
  "כללי) לפי שם הקופה. מסלולי המניות הגיאוגרפיים לא הגיעו לסף 8 הקופות בדי שנים, "
...
w(f"- סך הכול נבדקו **{H[1]['total']} אירועי איתות** על פני 6 קטגוריות "
  "(3 מסלולים × קרן השתלמות ו-3 מסלולים × קופת גמל).")
w("")
...
w(f"ה\"רודף\" הקדים את ה\"נשאר\" ב-{nwin} מתוך {len(gaps)} הקטגוריות, אך גודל הפער "
  "אינו אחיד: **במסלולים הכלליים** (בהם מרוכז רוב כספם של החוסכים) הפער זעום ואינו "
  f"מובהק סטטיסטית — כ-**{sg:+.2f} נק'** בקרן השתלמות כללי ו-**{pg:+.2f} נק'** בקופת "
  "גמל כללי — ואילו הפערים הגדולים יותר מופיעים במסלולי **המניות**, אך שם מספר שנות "
  "ההשוואה קטן (3–5 שנים בלבד) והפער אינו יציב.")
w("")
...
w(f"**מהנתונים:** המובילה יושבת בממוצע באחוזון תנודתיות ~{_pv} בתוך הקטגוריה — כלומר "
  "היא נוטה להיות מהקופות המסוכנות יותר בקבוצתה. הדבר בולט במסלולי המניות וגם "
  "במסלולים הכלליים ה'אגרסיביים' (חשיפת מניות גבוהה). מאחר שהתקופה הנבדקת כללה בעיקר "
  "שנים של עליות בשווקים, קופות בעלות חשיפה מנייתית גבוהה השיגו בממוצע תשואות גבוהות "
  ...
  "חזק', שהוביל בקופת גמל כללי ב-2010 וצנח למקום 72 מתוך 72 ב-2011).")
w("")
...
w(f"3. **היתרון הכספי אינו אחיד וקשור לסיכון.** במסלולים הכלליים — בהם מרוכז רוב "
  f"הכסף — הפער זעום ואינו מובהק (קרן השתלמות כללי {sg:+.2f} נק', קופת גמל כללי "
  f"{pg:+.2f} נק' לשנה); הפערים הגדולים יותר מופיעים במסלולי המניות (על מדגם קטן), "
  "היכן שהמובילה נוטה להיות הקופה המסוכנת יותר — כך שחלק ניכר מהפער משקף רמת סיכון "
