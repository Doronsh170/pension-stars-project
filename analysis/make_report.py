...
w("")
w("**עודכן:** יולי 2026 · **מקור:** גמל-נט (רשות שוק ההון) · "
...
w("המחקר מדמה חוסך שבכל תחילת שנה בוחן את טבלת התשואות של השנה שחלפה, מזהה את "
  ...
  "המחקר מתמקד בשנות איתות **מ-2010 ואילך**, ומפריד בין שתי משפחות מוצרים "
  "שהחוסך בוחר ביניהן: **קרן השתלמות** ו**קופת גמל** (תגמולים ואישית לפיצויים) "
  "— כל אחת בנפרד ובכל אחד ממסלולי ההשקעה שלה.")
w("")
w("### שלוש השאלות")
...
_n_hish = sum(1 for _, c in CAT_ORDER if c.startswith("קרנות השתלמות"))
_n_gemel = _n_study - _n_hish
w(f"- סך הכול נבדקו **{H[1]['total']} אירועי איתות** על פני {_n_study} קטגוריות "
  f"({_n_hish} מסלולים בקרן השתלמות ו-{_n_gemel} מסלולים בקופת גמל), לפי טיוב מסלולי "
  "ההשקעה לעמודת *מסלול ממופה*.")
w("")
...
eqg = gapof('קרנות השתלמות | מניות אקטיבי')
w("")
w(f"ה\"רודף\" הקדים את ה\"נשאר\" ב-{nwin} מתוך {len(gaps)} הקטגוריות, אך גודל הפער "
  ...
  f"השתלמות כללי ו-**{pg:+.2f} נק'** בקופת גמל כללי — ואילו הפער הגדול ביותר מופיע "
  f"במסלול **המניות האקטיביות** של קרן ההשתלמות (כ-**{eqg:+.2f} נק'**).")
w("")
...
w(f"**מהנתונים:** המובילה יושבת בממוצע באחוזון תנודתיות ~{_pv} בתוך הקטגוריה — כלומר "
  "היא נוטה להיות מהקופות המסוכנות יותר בקבוצתה. הדבר בולט במיוחד במסלולי המניות: "
  "המובילות הן לא פעם מסלולי מדד או עוקבי S&P 500. מאחר שהתקופה הנבדקת כללה בעיקר שנים "
  ...
  "של ביצועים. בשנים של ירידות, אותן מובילות צנחו לתחתית (למשל 'כלל איתן כללי דגש "
  "חזק', שהוביל בקופת גמל כללי ב-2010 וצנח למקום 66 מתוך 66 ב-2011).")
w("")
...
w(f"3. **היתרון הכספי אינו אחיד וקשור לסיכון.** במסלולים הכלליים — בהם מרוכז רוב "
  ...
  f"{pg:+.2f} נק' לשנה); הפער הגדול מופיע במסלולי המניות, היכן "
