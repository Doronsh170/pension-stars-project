...
# Categories actually present in the results, ordered family -> risk level.
...
def label(cat):
    """Family · risk level — the study's comparison group."""
...
w("המחקר מדמה חוסך שבכל תחילת שנה בוחן את טבלת התשואות של השנה שחלפה, מזהה את "
  ...
  "**באותה משפחת מוצר ובאותה רמת סיכון** — כך שקופה אינה \"מנצחת\" רק משום "
  "שלקחה יותר סיכון מהמתחרות שלה.")
w("")
w("### שלוש השאלות")
...
w("- **קטגוריה** = משפחת המוצר + **רמת הסיכון** של הקופה (למשל *קרן השתלמות · בינוני*). "
  "זו קבוצת ההשוואה הרלוונטית לחוסך: הוא בוחר קודם מוצר ורמת סיכון, ורק אז קופה. "
  "נדרשו לפחות 8 קופות בקטגוריה בשנת האיתות.")
w("- **רמת הסיכון** נלקחת מטבלת המיפוי `fund-risk-map.xlsx` — רמה אחת יציבה לכל קופה. "
  "עמודת `רמת סיכון` שבקובץ הנתונים עצמו אינה משמשת: היא מסווגת לפי **החשיפה המנייתית "
  "בשקלים** ולא לפי שיעורה מהנכסים, ולכן אותה קופה מחליפה רמות סיכון ככל שהיא גדלה. "
  "לקופות היסטוריות שאינן בטבלת המיפוי נגזרה הרמה מהתנהגות הקופה עצמה "
  "(`build_risk_levels.py`) — 90% התאמה מדויקת ו-99% בטווח רמה אחת מול הקופות הממופות.")
...
# Average gap per risk level, pooled across the four families.
gap_by_risk = {}
for r in RISK_ORDER:
    vals = [float(x["gap_annualized_pp"]) for x in strat
            if x["category"].split(" | ")[1] == r]
    if vals:
        gap_by_risk[r] = statistics.mean(vals)
lo_risk, hi_risk = RISK_ORDER[0], RISK_ORDER[-1]
w("")
w("הפער נערם באופן שיטתי לפי רמת הסיכון של הקטגוריה — ממוצע הפער השנתי בכל רמה, "
  "על פני ארבע משפחות המוצר:")
w("")
w("| רמת סיכון | פער שנתי ממוצע (רודף − נשאר) |")
w("|---|---:|")
for r in RISK_ORDER:
    if r in gap_by_risk:
        w(f"| {r} | {gap_by_risk[r]:+.2f} נק' |")
w("")
w(f"ה\"רודף\" הקדים את ה\"נשאר\" ב-{nwin} מתוך {len(gaps)} הקטגוריות, אך הפער "
  "אינו אחיד: הוא כמעט נעלם בקטגוריות הסולידיות — כ-"
  f"**{gap_by_risk[lo_risk]:+.2f} נק'** ברמת סיכון *{lo_risk}* — וגדל ככל שרמת "
  f"הסיכון עולה, עד כ-**{gap_by_risk[hi_risk]:+.2f} נק'** ברמת סיכון *{hi_risk}*. "
  "כלומר הרווח לכאורה של הרדיפה מרוכז דווקא בקטגוריות שבתוכן נותר עדיין מרחב "
  "סיכון גדול — ולא בקטגוריות שבהן כל הקופות דומות זו לזו.")
w("")
...
w(f"**מהנתונים:** גם לאחר שהשוואנו קופות באותה רמת סיכון, המובילה יושבת בממוצע "
  f"באחוזון תנודתיות ~{_pv} **בתוך** הקטגוריה — כלומר היא נוטה להיות מהקופות "
  "התנודתיות יותר בקבוצתה. רמת סיכון היא סרגל גס: בתוך רמה אחת עדיין נכנסות קופות "
  "בעלות אופי שונה, והמובילה נוטה להיות זו שבקצה הנועז של הרמה. מאחר שהתקופה "
  "הנבדקת כללה בעיקר שנים של עליות בשווקים, אותן קופות השיגו בממוצע תשואות גבוהות "
  "יותר — כך שחלק מהפער הכספי שבסעיף (א) משקף סיכון, ולא בהכרח התמדה של ביצועים. "
  "בשנים של ירידות אותן מובילות צנחו לתחתית הקטגוריה שלהן.")
w("")
...
w(f"1. **שמירת מעמד חלקית וזמנית.** רק כ-{H[1]['still1']:.0f}% מהמובילות חזרו למקום "
  ...
  f"בתוך שלוש שנים כ-{H[3]['vanished']:.0f}% מהמובילות נסגרו או מוזגו.")
...
w(f"3. **היתרון הכספי אינו אחיד — הוא עולה עם רמת הסיכון.** ברמות הסיכון "
  f"הסולידיות הפער כמעט נעלם ({gap_by_risk[lo_risk]:+.2f} נק' ברמה *{lo_risk}*), "
  f"וברמה *{hi_risk}* הוא מגיע לכ-{gap_by_risk[hi_risk]:+.2f} נק' לשנה. גם בתוך "
  f"רמת סיכון נתונה המובילה יושבת באחוזון תנודתיות ~{_pv} — כלומר חלק ניכר מהפער "
  "משקף סיכון עודף בתקופה של שווקים עולים, לא התמדה של ביצועים. הסימולציה גם "
  "אינה מנכה דמי ניהול, מס ועלויות מעבר.")
w("")
w("הנתונים המלאים והקוד בתיקיית `analysis/`. לקורא/ת נותרת ההחלטה כיצד לשקלל "
  "ממצאים אלה.")
w("")
w("---")
w("*שחזור: `build_risk_levels.py` → `build_annual.py` → `chase_analysis.py` → "
  "`risk_check.py` → `make_report.py` / `build_html.py`.*")
